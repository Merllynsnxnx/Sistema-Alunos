...
Class Disciplina {
    ...
    //função que verifica se essa disciplina pode ser cadastrada (permite duplicatas)
    public function valida_disciplina($nome) {
        // Não bloqueia disciplinas com o mesmo nome, apenas verifica se já existe um nome idêntico com o mesmo código
        $registros = $this->exibir(0, null, $nome, 0);

        $linhas = $registros->rowCount();

        if ($linhas > 0) {
            // Permite duplicação de nome, mas com códigos diferentes
            $msg = ("Essa disciplina já está registrada, mas você pode criar uma com código diferente.");
        } else {
    ...
    public function verificar_disponibilidade($codigo, $nome) {
        $registros = $this->exibir($codigo, null, $nome, 0);
